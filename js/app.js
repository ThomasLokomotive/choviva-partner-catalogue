  ...
  function render() {
    ...
    visibleProducers.forEach((producer) => {
      ...
      const logoHtml = producer.logo
        ? `<img class="producer-logo" src="${producer.logo}" alt="${escapeHtml(
            producer.name
          )} logo" loading="lazy" />`
        : "";
      head.innerHTML = `<div class="producer-title">${logoHtml}<h2>${escapeHtml(
        producer.name
      )}</h2></div><span class="category-pill">${escapeHtml(
      ...
      products.forEach((product) => {
        ...
        const imgHtml = product.img
          ? `<div class="product-img"><img src="${product.img}" alt="${escapeHtml(
              product.name
            )}" loading="lazy" /></div>`
